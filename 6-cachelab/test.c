...
void load(Cache *cache, int cache_len);
...
long long find_evi(Cache *cache, long long start_index){
    int min_num = cache[start_index].count;
    int evi_index = start_index;
    for(int i = start_index; i < start_index + num_e - 1; i++){
...
int main(int argc, char **argv)
{
    ...
    Cache *cache = (Cache*)malloc(12 * cache_len);//Cache数组，每一个元素就是一个缓存行
